...
const ProductContainerVertical = ({ Product }) => {



    return (
        <div className="pl-2 pr-2 containertext cursor-pointer mt-1 flex">
            {Product.map((item) => (
                <Link to={`/myProductdetails/${item._id}`} key={item._id} className="w-80 transform overflow-hidden rounded-lg bg-stone-600/30 text-stone-200 duration-300 hover:scale-105 m-2">
...
export default ProductContainerVertical;