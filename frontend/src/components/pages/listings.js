...
const Listings = () => {
  ...
  const fetchdata = async () => {
    setIsFetched(false);
    try {
      const res = await axios.get(`${backend}/api/listings/getlistings`, {
        params: filters,
      });
  ...
  const loadMore = () => {
    if (listings.length - limit.length > 8) {
      ...
      setLimit(listings.slice(0, newlimit));
    } else {
      setLimit(listings);
    }
  };
      ...
      >
        <div className="md:flex md:flex-wrap md:justify-between min-[150px]:text-center mt-10">
          <div className=" text-stone-200 min-[150px]:text-4xl min-[150px]:text-center md:text-5xl alluse md:pl-8 mb-5 md:mr-16">
            User <span className="text-yellow-600">Listings</span>
          </div>
        ...
        <div className="p-4 rounded-lg bg-neutral-900 ">
          ...
          <div className="flex flex-wrap justify-center">
            ...
            <Select
              value={filters.stones || undefined}
              className="mr-3"
              style={{ width: 200, height: 40, marginBottom: 20 }}
              onChange={(value) => setFilters({ ...filters, stones: value })}
              placeholder="Select Stones Type"
              options={[
                {
                  value: "None",
                  label: "None",
      ...
      <div className="categoriesarea text-stone-200 mb-5">
        <div className="categories flex flex-wrap pt-5 pb-5 ">
          {!isFetched ? (
            ...
            <div className="">{<ContainerAll listing={limit} />}</div>
          )}
        </div>
        {listings.length > limit.length ? (
          <div
            onClick={loadMore}
