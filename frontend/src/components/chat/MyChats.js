...
const MyChats = ({ fetchAgain }) => {
    ...
    const { selectedChat, setSelectedChat, user, chats, setChats, chatType, setChatType } = ChatState();
    ...
    const fetchAllJewelerDetails = async (chats) => {
        try {
            ...
            const responses = await Promise.all(ids.map(id => axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/jeweler/getuserjeweler/${id}`)));
    
            const jewelerDetails = responses.map(response => response.data.storename);
    ...
    const fetchUserChats = async () => {
        try {
            ...
            const { data } = await axios.get(`${REACT_APP_BACKEND_URL}/api/chat`, config);
            const userChats = data.filter(chat => chat.chattype === 'user' || (chat.chattype === 'jeweler' && chat.users[0]._id === user._id));
            setChats(userChats);
                ...
                {chats ? (
                    <Stack overflowY="scroll" className="max-h-[90%]">
                        {chats.map((chat, index) => (
                            ...
                            <div className={`mt-4 cursor-pointer alluse ${selectedChat === chat ? 'bg-yellow-600 text-stone-200' : 'bg-zinc-700 text-stone-200'} ${chat.seen.status === false && String(chat.seen.receiver) === String(user._id) ? 'bg-primary-600' : ''} px-4 py-3 rounded-lg md:mr-4 md:ml-4 md:text-2xl duration-300 hover:scale-105`} key={chat._id} onClick={() => setSelectedChat(chat)}>

                                <div className="flex">
                                {chat.chattype === 'jeweler' && user.status === 'user' ? (
                        <img src="/images/shopwhite.png" alt="user" className="min-[150px]:h-5 min-[150px]:w-5 md:h-7 md:w-7 mr-3" />
                    ) : (
                        <img src="/images/usericon.png" alt="user" className="min-[150px]:h-6 min-[150px]:w-6 md:h-8 md:w-8 mr-2" />
                    )}
                    <p>{chat.chattype === 'jeweler' && user.status === 'user' ? getInitials(jewelerDetails[index]) : getInitials(getSender(user, chat.users))}</p>
