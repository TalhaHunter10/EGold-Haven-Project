...
const Container = ({ listing }) => {
    ...
    return (
        <div className="pl-2 pr-2 containertext cursor-pointer mt-1 pb-10 pt-5 flex">
            {listing.map((item) => (
                <div className="w-80 transform overflow-hidden rounded-lg bg-stone-600/30 text-stone-200 duration-300 hover:scale-105 m-2" key={item._id}>
                    {item.status !== "sold" ? (
                        <Link to={`/mylistingdetails/${item._id}`} className="block">
