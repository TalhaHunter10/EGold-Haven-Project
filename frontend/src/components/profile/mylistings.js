...
const Mylistings = () => {
    ...
    const soldListings = listing.filter(item => item.status === "sold");
    const activeListings = listing.filter(item => item.status !== "sold");
                ...
                listing.length === 0 ? (
                    ...
                    <div>
                        <div className="flex flex-wrap justify-start">
                            <Container listing={activeListings} />
                        </div>
                        {soldListings.length > 0 && (
                            <div className="alluse">
                                <h2  className="alluse text-4xl text-stone-200 pb-8">Sold Listings</h2>
                                <Container listing={soldListings} />
                            </div>
                        )}
                    </div>
                )
