...
const getStats = asyncHandler(async (req, res) => {
  const [
    ...
    registeredJewelersCount,
    pendingJewelerRequestsCount,
    liveListingsCount,
    soldListingsCount,
...
const getJewelerRequests = asyncHandler(async (req, res) => {
  const jewelers = await Jeweler.find({
    user: { $in: await User.find({ status: "requested" }).distinct("_id") },
  });
...
const acceptJeweler = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    const notificationData = {
      notification:
        "Congratulations ! Your request for Jeweler status has been accepted. You can now manage your store.",
      status: "unread",
      receivertype: "user",
      notificationtype: "jewelerstore",
      receiver: userId,
    };
    ...
    res
      .status(200)
      .json({ message: "Jeweler Status Accepted Successfully !!!" });
  } else {
    res.status(404);
...
const rejectJeweler = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    const notificationData = {
      notification:
        "Your request for Jeweler status has been rejected. Please try again later.",
      status: "unread",
      receivertype: "user",
    ...
    res
      .status(200)
      .json({ message: "Jeweler Status Request Rejected Successfully !!!" });
  } else {
    res.status(404);
    throw new Error("User not found");
  }
...
const acceptListing = asyncHandler(async (req, res) => {
  ...
  if (listing) {
    ...
    const notificationData = {
      notification: "Your listing has been approved and is now live.",
      status: "unread",
      receivertype: "user",
    ...
    res
      .status(200)
      .json({ message: "Listing Status Accepted Successfully !!!" });
  } else {
    res.status(404);
...
const rejectListing = asyncHandler(async (req, res) => {
  ...
  if (listing) {
    ...
    const notificationData = {
      notification: "Your listing has been rejected. Edit and resubmit.",
      status: "unread",
      receivertype: "user",
      notificationtype: "mylisting",
      receiver: listing.user,
    ...
    res
      .status(200)
      .json({ message: "Listing Status Rejected Successfully !!!" });
  } else {
    res.status(404);
    throw new Error("Listing not found");
  }
...
const acceptProduct = asyncHandler(async (req, res) => {
  ...
  if (product) {
    ...
    const notificationData = {
      notification: "Your product has been approved and is now live.",
      status: "unread",
    ...
    res
      .status(200)
      .json({ message: "Product Status Accepted Successfully !!!" });
  } else {
    res.status(404);
...
const rejectProduct = asyncHandler(async (req, res) => {
  ...
  if (product) {
    ...
    const notificationData = {
      notification: "Your product has been rejected. Edit and resubmit.",
      status: "unread",
      receivertype: "jeweler",
      notificationtype: "jewelerstore",
      receiver: product.user,
    ...
    res
      .status(200)
      .json({ message: "Product Status Rejected Successfully !!!" });
  } else {
    res.status(404);
    throw new Error("Product not found");
  }
...
const getCommissionChangeRequests = asyncHandler(async (req, res) => {
  const commissionRequests = await CommissionRequest.find({
    status: "pending",
  });
...
const acceptCommissionChangeRequest = asyncHandler(async (req, res) => {
  ...
  if (commissionRequest) {
    ...
    if (jeweler) {
      ...
      const notificationData = {
        notification: "Your request for commission change has been approved.",
        status: "unread",
        receivertype: "jeweler",
        notificationtype: "jewelerstore",
      ...
      res.status(200).json({
        message: "Commission Change Request Accepted Successfully !!!",
      });
    } else {
      res.status(404);
      throw new Error("Jeweler not found");
...
const rejectCommissionChangeRequest = asyncHandler(async (req, res) => {
  ...
  if (commissionRequest) {
    ...
    const notificationData = {
      notification: "Your request for commission change has been rejected.",
      status: "unread",
      receivertype: "jeweler",
      notificationtype: "jewelerstore",
      receiver: commissionRequest.jeweler,
    };
    ...
    res
      .status(200)
      .json({ message: "Commission Change Request Rejected Successfully !!!" });
  } else {
    res.status(404);
    throw new Error("Commission Request not found");
  }
