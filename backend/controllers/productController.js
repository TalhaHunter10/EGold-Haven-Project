...
const createProduct = asyncHandler(async (req, res) => {
  const { title, price, description, status, category, karats, weight } =
    req.body;
  ...
  const notificationData = {
    notification: "Your product is now pending approval !",
    status: "unread",
    receivertype: "jeweler",
    notificationtype: "null",
    receiver: [jeweler._id],
  };
...
const getLiveProducts = asyncHandler(async (req, res) => {
  const liveProducts = await Product.find({ status: "live" }).sort({
    createdAt: -1,
  });
  ...
  const productsWithJewelerNames = liveProducts.map((product) => {
    const jeweler = jewelers.find((jeweler) =>
      jeweler._id.equals(product.jeweler)
    );
...
const deleteProduct = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const notificationData = {
      notification: "Your product has been deleted successfully!",
      status: "unread",
      receivertype: "jeweler",
      notificationtype: "null",
      receiver: [product.jeweler],
    };
...
const getJewelerProducts = asyncHandler(async (req, res) => {
  ...
  const jewelerProducts = await Product.find({ jeweler: jeweler._id }).sort({
    createdAt: -1,
  });
...
const getJewelerProductsInformation = asyncHandler(async (req, res) => {
  ...
  const products = await Product.find({ jeweler: id, status: "live" }).sort({
    createdAt: -1,
  });
  ...
  try {
    ...
    if (location) {
      ...
      const filteredProducts = products.filter(
        (product) => product.distance <= maxDistance
      );
